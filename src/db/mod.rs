...
fn with_conn<T, F: FnOnce(&Connection) -> Result<T>>(f: F) -> Result<T> {
    match DB_CONN.lock() {
        Ok(conn) => f(&*conn),
        Err(_) => {
            // Mutex poisoned: fall back to one-shot connection
            let conn = open_conn()?;
            f(&conn)
        }
    }
    ...
    with_conn(|conn| {
        ...
        let has_column = {
            ...
            let rows: Vec<String> = stmt
                ...
                .filter_map(|r| r.ok())
                .collect();
            rows.contains(&"claude_session_id".to_string())
        };
        ...
        if !has_column {
            ...
            info!("Migrated: added claude_session_id column to sessions table");
        }
