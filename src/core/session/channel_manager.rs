...
impl ChannelManager {
    ...
    /// Persist `provider_session_id` on the gateway agent record (all platforms / WebUI).
    pub fn persist_provider_session_id(
        &self,
        agent_record_id: &str,
        provider_session_id: Option<String>,
    ) {
        let Some(mut session) = self.get_agent_session(agent_record_id) else {
            return;
        };
        let unchanged = session.provider_session_id == provider_session_id;
        if unchanged {
            return;
        }
    ...
    pub fn create_agent_session_only(
        ...
        let session = AgentSession {
            ...
            title: crate::session::channel_model::truncate_agent_session_title(title),
    ...
    /// Record an already-active agent session into memory and DB.
    ...
    /// Create the gateway agent record and link the controller **before** spawning the provider
    /// (so `provider_session_id` can persist during spawn).
    pub async fn prepare_controller_agent_record(
        &self,
        channel_id: &str,
        title: &str,
        ...
        let provider = {
            let ctrl = controller.lock().await;
            let name = ctrl.provider_name().await;
            drop(ctrl);
            if name.trim().is_empty() {
                self.effective_channel_provider(channel_id, &profiles)
                    .to_string()
            } else {
                name
            }
        ...
        self.stop_active_session_records_for_channel(channel_id, None);
        let session =
            self.record_active_agent_session(channel_id, title, &work_dir, &provider, None)?;
        {
            let ctrl = controller.lock().await;
            ctrl.set_linked_agent_record_id(Some(session.id.clone()))
                .await;
        }
        Ok(session)
    }

    /// After the provider subprocess is up, sync work_dir / provider / provider_session_id onto the row.
    pub async fn finalize_controller_agent_record(
        &self,
        ...
        drop(ctrl);
        self.refresh_agent_session_from_controller(agent_session_id, controller)
            .await;
        let ctrl = controller.lock().await;
        ctrl.sync_provider_session_id_to_record().await;
        Ok(())
    }

    pub fn deactivate_agent_record(&self, agent_session_id: &str) {
    ...
    pub async fn record_active_controller_session(
        ...
        if let Some(existing) = self.get_active_agent_session(channel_id) {
            self.finalize_controller_agent_record(&existing.id, controller)
                .await?;
            return Ok(Some(existing));
        }

        let session = self
            .prepare_controller_agent_record(channel_id, title, controller)
            .await?;
        self.finalize_controller_agent_record(&session.id, controller)
            .await?;
        Ok(Some(session))
    }
    ...
    pub async fn create_and_start_agent_session(
        ...
        {
            let ctrl = controller.lock().await;
            ctrl.set_linked_agent_record_id(Some(agent_session_id.clone()))
                .await;
        ...
        let session = AgentSession {
            ...
            provider,
            title: crate::session::channel_model::truncate_agent_session_title(&args.title),
            work_dir,
            active: true,
            state: AgentSessionState::Active,
            provider_session_id: provider_session_id.clone(),
    ...
    pub async fn resume_agent_session_for_platform<C: Into<AgentProfiles>>(
        ...
        provider_override: Option<AgentProvider>,
    ) -> Result<ActiveAgentRuntime> {
        let agent_settings = agent_settings.into();
        let existing = self.get_agent_session(session_id).ok_or_else(|| {
            anyhow::anyhow!(
                "{}",
        ...
        let spawn_provider = provider_override
            .clone()
            .unwrap_or_else(|| stored_provider.clone());

        if !agent_settings.is_provider_enabled(&spawn_provider) {
            return Err(anyhow::anyhow!(
                "{}",
                crate::t_fmt!(
                    "session.provider_disabled",
                    NAME = spawn_provider.to_string()
                )
            ));
        }
        ...
        let resume_same_provider = spawn_provider == stored_provider;
        {
            let ctrl = controller.lock().await;
...
fn should_try_provider_resume(agent_session: &AgentSession, provider: &AgentProvider) -> bool {
    ...
    if !crate::command::agents::provider_supports_session_resume(provider) {
        return false;
    }
    match provider {
        AgentProvider::Pi
...
pub(crate) fn gateway_session_has_user_history(agent_session: &AgentSession) -> bool {
    ...
    let file_path = home
        .join(".cc-gateway")
        .join("history")
        .join(format!("{}.jsonl", agent_session.id));
...
mod gateway_history_tests {
    ...
    #[test]
    fn gateway_history_uses_agent_session_id_not_provider_session_id() {
        use crate::tests::helpers::TestEnv;
        // Serialize with other HOME-mutating tests via TestEnv's ENV_LOCK; raw
        // `set_var("HOME")` here previously raced the config/db tests in parallel.
        let env = TestEnv::new();
        let hist = env.home().join(".cc-gateway/history");
        std::fs::create_dir_all(&hist).unwrap();
        std::fs::write(
            hist.join("provider-sid.jsonl"),
            r#"{"role":"user","content":"wrong file"}"#,
        )
        .unwrap();

        let session = AgentSession {
            id: "gateway-uuid".to_string(),
            provider_session_id: Some("provider-sid".to_string()),
            ..AgentSession::new("ch", "t", "/tmp")
        };

        assert!(!gateway_session_has_user_history(&session));
        std::fs::write(
            hist.join("gateway-uuid.jsonl"),
