...
pub async fn list_models_via_cli(provider: &AgentProvider, config: &AgentConfig) -> Result<Vec<String>> {
    match provider {
        AgentProvider::Cursor => list_cursor_models(config).await,
        AgentProvider::OpenCode => list_opencode_models(config).await,
        AgentProvider::Claude => Ok(curated_claude_models().iter().map(|s| s.to_string()).collect()),
