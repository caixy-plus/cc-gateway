...
优雅地关闭守护进程。所有活跃的聊天会话都会收到关闭通知，每个智能体子进程有 500 毫秒的时间退出，超时将被强制终止。
...
1. 打开飞书并找到你的机器人
2. 直接发送消息 — 会话激活时转发给当前智能体
3. 使用网关命令: `/cd`, `/agent`, `/agents`, `/agent-history`, `/pwd`, `/ll`, `/help`, `/quit`

每个聊天 (群聊或私聊) 都有独立的智能体子进程，不同聊天的消息不会相互混淆。

### 目录选择卡片

在飞书中发送 `/ll` 可收到交互式卡片，列出**当前**工作目录下的子文件夹（初始为 `default_dir`）。点击按钮即可 `cd`。

### 命令边界

- `/cd`（全平台）：解析后的路径须落在**用户主目录**内；`/cd ..` 不能跳出主目录
- `/quit` 仅在智能体会话激活时有效；否则会提示先启动会话
