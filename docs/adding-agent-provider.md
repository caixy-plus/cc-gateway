...
| **F. User-facing registry** | `src/core/config/agent_registry.rs`, `src/core/command/agents.rs` | Add `AgentProviderDef` with [`AgentCapabilities`](../../src/core/config/agent_registry.rs) (`session_resume`, `context_compact`, `memory_init`, `platform_bound`, `list_models`, `in_session_model_switch`, `model_switch_via_user_message`, `mcp`, …). `provider_supports_*` and MCP matrix read from registry — do not add new hard-coded `match` arms elsewhere. |
