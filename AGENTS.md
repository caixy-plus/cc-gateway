...
- **`core/agent/backend.rs`**: Unified gateway-facing session API (`send_user_message`, `set_model`, `compact_context`, …). Codex stream-json, Pi RPC, and ACP sessions each implement `AgentBackend`.
- **`core/runtime/session.rs`** / **`core/runtime/protocol.rs`**: Codex **stream-json** over stdio (`StreamJsonSession` implements `AgentBackend`).
...
- Codex: after spawning the `Codex` subprocess, cc-gateway reads `~/.Codex/sessions/{pid}.json` to extract Codex's internal session id, persists it as `provider_session_id`, and uses it to resume when supported.
...
- **Stream-json protocol**: All Codex communication is newline-delimited JSON. Each line is one event. Codex must be launched with `--input-format stream-json --output-format stream-json`.
